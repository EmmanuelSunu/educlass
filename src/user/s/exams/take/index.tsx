    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md text-center">
          ...
          <h2 className="text-xl font-semibold text-red-700 mb-2">Exam Unavailable</h2>
          <p className="text-gray-600 mb-4">This exam is no longer available as the due date has passed.</p>
          <button 
            onClick={() => navigate('/user/s/exams')}
...
const StudentExamTake: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      if (foundExam) {
        ...
        const totalSeconds = (foundExam.durationHours * 60 * 60) + 
                             (foundExam.durationMinutes * 60);
  ...
  const submitExam = () => {
    // Here you would normally send the responses to your backend
    console.log('Exam submitted', responses);
    navigate(`/user/s/results/${id}`);
  };
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-2 bg-blue-50 p-2 rounded">
            ...
            <span className={`font-mono ${timeLeft < 300 ? 'text-red-500' : ''}`}>
              {examStarted ? formatTime(timeLeft) : exam.duration}
            </span>
          </div>
        </div>
        ...
        {!examStarted ? (
          <div className="bg-white p-6 rounded-lg shadow mb-6">
            ...
            <ul className="list-disc pl-5 mb-6 space-y-2">
              ...
              <li>Answer all questions to the best of your ability.</li>
              <li>You cannot copy, paste, or drag content during the exam.</li>
          ...
          <>
            <div className="space-y-8 mb-6">
              {exam.questions.map((question, index) => (
                <div key={question.id} className="bg-white p-6 rounded-lg shadow exam-question">
                  <div className={`question-type-badge question-type-${question.type}`}>
                    {question.type}
                  </div>
                  ...
                  {question.type === 'multi-choice' && question.options && (
                    <div className="space-y-2">
                      {question.options.map((option, optIndex) => (
                        <label key={optIndex} className="flex items-start space-x-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                          <input
                            type="radio"
                            name={`question-${question.id}`}
                            value={option}
                  ...
                  {question.type === 'fill-ins' && (
                    <input
                      ...
                      className="w-full p-2 border rounded no-drag-drop"
                      spellCheck="false"
                      autoCorrect="off"
                      draggable="false"
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-gray-500">
                {Object.keys(responses).length} of {exam.questions.length} questions answered
              </div>
              <button
                onClick={submitExam}
