  ...
  const renderStudentAnswer = (question: ResultQuestion) => {
    return (
      <div className="mt-2">
        ...
        <div className="p-3 bg-gray-50 rounded mt-1 text-slate-700">{question.yourAnswer}</div>
        
        <div className="font-medium text-slate-700 mt-3">Correct Answer:</div>
        <div className="p-3 bg-gray-50 rounded mt-1 text-slate-700">{question.correctAnswer}</div>
    ...
    <DashboardLayout>
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-sm p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">{results.examTitle}</h1>
            <p className="text-slate-600">{results.examClass}</p>
          </div>
          <span className={`px-3 py-1 text-sm font-medium rounded-full ${scoreBadge.color}`}>
            {scoreBadge.text}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg mb-6">
          ...
          <div>
            ...
            <p className="font-medium text-gray-700">{new Date(results.submittedAt).toLocaleDateString()}</p>
          </div>
          <div>
            <p className="text-sm text-gray-500">Correct Answers</p>
            <p className="font-medium text-gray-700">{results.correctAnswers} of {results.totalQuestions}</p>
          </div>
        </div>
