...
import { FiBook, FiBookOpen, FiBookmark, FiTrendingUp } from 'react-icons/fi';
import DashboardLayout from '../../layout';
import { getCourseStats } from '../../data/analytics';
...
const CourseAnalytics = () => {
  ...
  const ProgramDistribution = () => (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h3 className="text-lg font-semibold text-slate-800 mb-4">Courses by Program</h3>
      <div className="space-y-4">
        {stats.coursesByProgram.map((program) => (
          <div key={program.name}>
            <div className="flex justify-between text-sm mb-1">
              <span className="font-medium text-slate-700">{program.name}</span>
              <span className="text-slate-600">{program.count} courses</span>
            </div>
  ...
  const ActivityChart = () => (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h3 className="text-lg font-semibold text-slate-800 mb-4">Course Activity</h3>
      <div className="h-64 flex items-end justify-between gap-2">
        {stats.courseActivity.map((day) => (
          <div key={day.date} className="flex-1">
            <div className="relative h-full flex flex-col justify-end">
              <div
                className="w-full bg-primary/20 rounded-t"
                style={{
                  height: `${(day.active / stats.totalCourses) * 100}%`,
                }}
              />
              <div
                className="w-full bg-primary rounded-t absolute bottom-0"
                style={{
                  height: `${(day.new / stats.totalCourses) * 100}%`,
                }}
              />
            </div>
      ...
      <div className="flex items-center justify-center gap-4 mt-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-slate-600">New Courses</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <DashboardLayout title="Course Analytics" showAddButton={false}>
      <div className="space-y-6">
        {/* Time Range Selector */}
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">Course Overview</h2>
          <select
            value={timeRange}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <StatCard
            ...
            description="Courses active this week"
          />
          <StatCard
            title="New Courses"
